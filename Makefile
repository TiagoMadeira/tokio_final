...
start_production_local: start_minikube start_jaeger_server start_local_production_env expose_ingress_controller
...
expose_ingress_controller:
	...
	@if sudo lsof -i :80 -i :443 >/dev/null 2>&1; then \
		...
		sudo -E minikube kubectl -- port-forward service/ingress-nginx-controller -n ingress-nginx 80:80 443:443 & \
	fi
...
setup: install_basic_deps install_CI_dependencies install_node install_docker install_minikube install_trivy add_hosts

install_basic_deps:
	@echo "Updating packages and installing basic dependencies..."
	sudo apt-get update -y
	sudo apt-get install -y curl apt-transport-https ca-certificates gnupg lsb-release python3.10-venv

 install_node:
	curl -fsSL https://deb.nodesource.com/setup_22.x | sudo -E bash -
	# Clean update and fetch the exact Node 22 build via apt-get
...
launch_staging: start_and_prepare_staging run_vulnerability_tests run_tests sonar_scan clear_staging
...
prepare_staging_for_integration_testing:
	...
	minikube kubectl -- port-forward service/rest-internal-service 8000:80 -n staging &
	minikube kubectl -- port-forward service/frontend-internal-service 3000:80 -n staging &
...
audit_rest_service:
	...
	pip-audit -r requirements-stg.txt || true; stty sane
...
rest_service_tests:
	...
	PYTHONPATH=rest_service ENABLE_MONOTORING=False rest_service/.venv/bin/pytest rest_service/tests/ --cov=rest_service/app -W ignore --cov-report=xml:rest_service/coverage.xml

post_service_tests:
	python3 -m venv "post_service/.venv"
	post_service/.venv/bin/pip install -r post_service/requirements-stg.txt
	PYTHONPATH=post_service ENABLE_MONOTORING=False post_service/.venv/bin/pytest post_service/tests/ --cov=post_service/app -W ignore --cov-report=xml:post_service/coverage.xml

auth_service_tests:
	python3 -m venv "auth_service/.venv"
	auth_service/.venv/bin/pip install -r auth_service/requirements-stg.txt
	PYTHONPATH=auth_service ENABLE_MONOTORING=False SECRET_KEY=$(JWT_SECRET) auth_service/.venv/bin/pytest auth_service/tests/ -W ignore --cov=auth_service/app --cov-report=xml:auth_service/coverage.xml
